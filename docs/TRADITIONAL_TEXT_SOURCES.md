...
Currently, the generated yearly Panchang contract reports **334 unique festival identities** and **126 unique vrat identities**. These identity totals are distinct from dated occurrence counts; repeated observances such as monthly vrats are counted once per canonical identity. Recent festival-module updates also cleaned up overlapping identities, merged some alias-only observances into their canonical families, added the generic monthly Vratni Purnima identity, added Shree Hari Antardhan (Swadhaam Gaman), extended monthly Hari Jayanti into intercalary (Adhika) months, refreshed the vrat catalog, and corrected a set of rule-specific decision paths.
...
| Calendar period windows | Generated today/month/raw outputs expose **nirayana (sidereal)** and **sayana (tropical)** ayana/ṛtu windows plus samvat, samvatsara, and lunar-month windows | `OutputGeneratorService`, `PanchangService` | Day-detail payloads expose `Ayana`/`Ritu` (nirayana), explicit `Nirayana_*`, `Sayana_Ayana`/`Sayana_Ritu`, locale-stable `*_Key` fields, system labels, and `Mahadiksha_Guidance` where included. |
...
| Nag Panchami calendar-specific handling | Amanta output follows the Shravan Krishna Panchami / Nag Pancham convention, while Purnimanta output follows the Shravan Shukla Panchami convention | `FestivalService::FESTIVALS`, `FestivalRuleEngine` | Encoded with calendar-aware paksha fields and Nag Panchami paraviddha metadata. Goga Pancham remains a separate Gujarat-facing observance identity. |
| Vratni Purnima / named Purnima Vrats | Generic monthly Vratni Purnima follows the 18 fixed-ghadi Chaturdashi condition; named Purnima vrat identities use their configured Purnima vrat rule | `FestivalService::FESTIVALS`, `FestivalRuleEngine` | This separates monthly Purnima-vrat decision logic from sunrise-vyapini Mukutotsav Purnima and other festival-only Purnima labels. |
...
| Eclipse sutak | Lunar: 3 prahar lookback; Solar: 4 prahar lookback, with local visibility and ritual-magnitude gating | `EclipseService::sutak()` | Prahar boundaries are dynamic, using local sunrise/sunset day and night divisions. Sutak is not applicable when the eclipse is not ritually visible at the location. Outputs also expose Grast Uday/Grast Ast classification, Chudamani Yoga, telescope-only status, and post-eclipse bath/fresh-food guidance. |
...
| `PanchakCalculator.php` | `calculatePanchak()` | **Published Panchang convention:** Moon in Dhanishtha pada 3 through Revati (`rule_system = moon_dhanishta_pada_3_to_revati`). Active Panchak windows also expose entry-weekday subtype classification (`weekday_type_rule_system = panchak_start_weekday_type_table`): Sunday Roga Panchaka, Monday Raja Panchaka, Tuesday Agni Panchaka, Friday Chora Panchaka, Saturday Mrityu Panchaka, and Wednesday/Thursday Shubha Panchaka or no named five-dosha subtype. |
...
- `Sandhya` uses fixed-ghati offsets around actual local sunrise, solar noon, and sunset. It is distinct from the proportional fivefold daytime divisions.
...
- `Pradosha_Kaal` begins at actual local sunset and extends for six fixed ghatis. Trayodashi overlap determines the effective observance window.
- `Varjyam` and `Amrita_Kaal` are live-timed Panchang-day outputs. `Amrita_Kaal` is calculated from nakshatra-specific Amrita ghati offsets and is not derived from Varjyam.
