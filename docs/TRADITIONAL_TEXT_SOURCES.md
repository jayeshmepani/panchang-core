...
Currently, the generated yearly Panchang contract reports **324 unique festival identities** and **120 unique vrat identities**. These identity totals are distinct from dated occurrence counts; repeated observances such as monthly vrats are counted once per canonical identity. Recent festival-module updates also cleaned up overlapping identities, merged some alias-only observances into their canonical families, refreshed the vrat catalog, and corrected a set of rule-specific decision paths.
