...
| Calendar period windows | Generated today/month/raw outputs expose sidereal and Sayana ayana/ritu windows plus samvat, samvatsara, and lunar-month windows | `OutputGeneratorService`, `PanchangService` | Day-detail payloads expose localized `Sayana_Ayana`, `Sayana_Ritu`, and `Mahadiksha_Guidance` where included. |
